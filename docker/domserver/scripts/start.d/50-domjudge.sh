...
if grep -q TRUSTED_PROXIES webapp/.env.local >/dev/null 2>&1; then
...
for TRUSTED_PROXY in "${TRUSTED_PROXIES_ARRAY[@]}"; do
...
if [[ ! -f webapp/config/load_db_secrets.php ]]; then
...
if [[ -f webapp/.env.local.php ]]; then
	chown www-data: webapp/.env.local.php
fi
echo "[ok] Generated credential files"
echo
...
while [ $DB_UP -gt 0 ]; do
	...
	if ! mysqlshow -u"${MYSQL_USER}" -p"${MYSQL_PASSWORD}" -h"${MYSQL_HOST}" "${MYSQL_DATABASE}" >/dev/null 2>&1; then
...
if ! bin/dj_setup_database -uroot -p"${MYSQL_ROOT_PASSWORD}" status >/dev/null 2>&1; then
	echo "  Database not installed; installing..."
	INSTALL=install
	if [ "${DJ_DB_INSTALL_BARE}" -eq "1" ]; then
	...
	if [ "${admin_pw_file_existed}" -eq "0" ] && [[ -f etc/initial_admin_password.secret ]]; then
	...
	if [ "${restapi_secret_file_existed}" -eq "0" ]; then
...
if grep -q "^# NOTE(password-mismatch)" <etc/restapi.secret; then
...
if [[ -d /scripts/post-start.d/ ]]; then
	for i in /scripts/post-start.d/*; do
		if [[ -x "$i" ]]; then
